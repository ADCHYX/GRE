...
class Benchmark {
    ...
    bool insert_file = false;
    std::string insert_file_path;
    std::string insert_file_type;
...
#pragma omp parallel for num_threads(thread_num)
  ...
  */
    inline void parse_args(int argc, char **argv) {
      ...
      operations_num = stoll(get_with_default(flags, "operations_num", "1000000000")); // required
      ...
      insert_file = get_boolean_flag(flags, "insert_file");
      if(insert_file)
      {
        insert_file_path = get_required(flags, "insert_file_path"); // required
        insert_file_type = get_with_default(flags, "insert_file_type", "binary");
      }

      COUT_THIS(
          "[micro] Read:Insert:Update:Scan:Delete= " << read_ratio << ":" << insert_ratio << ":" << update_ratio << ":"
    ...
    void generate_operations(KEY_TYPE *keys) {
      ...
      KEY_TYPE *insert_keys = nullptr;
      long long insert_file_size = -1;
      if(insert_file)
      {
        COUT_THIS("Reading insert keys from other file.");
        if (insert_file_type == "binary") {
          insert_file_size = load_binary_data(insert_keys, insert_file_size, insert_file_path);
          if (insert_file_size <= 0) {
            COUT_THIS("Could not open key file, please check the path of key file.");
            exit(0);
          }
        } else if (insert_file_type == "text") {
          insert_file_size = load_text_data(insert_keys, insert_file_size, insert_file_path);
          if (insert_file_size <= 0) {
            COUT_THIS("Could not open key file, please check the path of key file.");
            exit(0);
          }
        ...
        size_t len1 = init_keys.size();
        size_t len2 = insert_file_size;
        unique_data(init_keys.data(), len1, insert_keys, len2);
        COUT_VAR(insert_file_size);
      }
      ...
      for (size_t i = 0; i < operations_num; ++i) {
        ...
        if (prob < read_ratio) {
          ...
          //operations.push_back(std::pair<Operation, KEY_TYPE>(READ, keys[rand()%(insert_counter-100)]));
          ...
          if(insert_file)
          {
            if(insert_counter >= insert_file_size)
              break;
            else
              operations.push_back(std::pair<Operation, KEY_TYPE>(INSERT, insert_keys[insert_counter++]));
          }
          else
